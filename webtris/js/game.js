class Game {
    ...
    checkForClearedLines() {
        ...
        outer: for (let y = this.arena.length - 1; y > 0; y--) {
            ...
            if (scoreMultiplier === 40) scoreMultiplier = 100;
            else if (scoreMultiplier === 100) scoreMultiplier = 300;
            else if (scoreMultiplier === 300) scoreMultiplier = 1200;
        ...
        matrix.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value !== 0) {
                    ...
                    context.fillRect(
                        (x + offset.x) * blockSize + innerRectOffset,
                        (y + offset.y) * blockSize + innerRectOffset,
                        blockSize - innerRectOffset * 2,
                        blockSize - innerRectOffset * 2
                    );
    ...
    resetPiece() {
        ...
        this.currPiecePos.x = Math.floor(
            (this.arena[0].length - this.currPiece[0].length) / 2
        );
